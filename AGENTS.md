...
- Publication policy updated 2026-09-25: finish the experiment stages, summarize and report results to the user, then publish the complete batch to `main` at `https://github.com/PigeonLabs/KNU_Capstone1_VAD` only after explicit approval for that batch. Local logs/snapshots/commits may continue. Keep the Korean README evidence-based. Never force-push or overwrite unrelated remote work.
