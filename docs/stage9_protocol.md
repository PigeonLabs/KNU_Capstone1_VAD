# 9단계 — 양자화 실행 경로·컴파일·정상 특징 진단

9단계는 DINOv2 양자화와 실행 경로를 진단하는 단계이며, 8단계 LoRA 실험과 별개다. 앞서 수행한 정상 96프레임 probe는 사전 진단으로 포함한다. 본 단계는 연산 경로, 컴파일, 자원 사용량, 정상 특징의 수치 차이를 측정한다. 전체 테스트 AUROC 산출과 테스트 결과를 이용한 모델 선택은 범위에 포함하지 않는다.
