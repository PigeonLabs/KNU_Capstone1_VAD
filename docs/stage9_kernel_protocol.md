# 9단계 후속 — 선형층 커널·캐시 병목 진단

이 후속 진단은 실제 선형층별 BF16·INT4 커널, 압축 해제 비용, 메모리 대역폭과 Tensor Core 활용률을 비교한다. 기존 결과를 보존하고 `kernel_study`에 기록하며, 새 VAD 학습이나 테스트 AUROC 평가는 포함하지 않는다.
